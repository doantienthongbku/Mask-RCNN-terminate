...
def compute_rpn_class_loss(rpn_match, rpn_class_logits):
    """RPN anchor classifier loss by cross entropy loss.

    Args:
        rpn_match: [batch, num_anchors, 1]. Anchor match type. 1=positive,
                -1=negative, 0=neutral anchor.
        rpn_class_logits: [batch, num_anchors, 2]. RPN classifier logits for FG/BG.
    """

    # Squeeze last dim to simplify
    rpn_match = rpn_match.squeeze(axis=2)   # [batch, num_anchors]

    # get anchors class. Convert the -1/+1 match to 0/1 values.
...
def compute_rpn_bbox_loss(target_bbox, rpn_match, rpn_bbox):
    """Return the RPN bounding box loss graph by Smooth L1 loss
